...
def send_confirmation_email(sender_email, receiver_email, sign_id):
    """
    ...
    # Sender email
    html_message_sender = render_to_string("confirmation.html", context=context)
    plain_message_sender = strip_tags(html_message_sender)

    message_sender = EmailMultiAlternatives(
        subject="Sign Request Document Signed", 
        body=plain_message_sender,
        from_email=None,
        to=[sender_email]
    )

    message_sender.attach_alternative(html_message_sender, "text/html")
    message_sender.send()
